#!/bin/bash
# generate_best_pae.sh — Run from root dir containing 15mer/, 18mer/, etc.
...
for variant_dir in */; do
  ...
  file=$(find_best_conf "$variant_dir"'replacement/*/boltz_results_*/predictions/*/confidence_*.json')
  if [ -n "$file" ] && [ -f "$file" ]; then
    ...
    [ -n "$pae" ] && echo "${variant}|replacement|${pae}|${file}|${plddt_f}|${cif}" >> "$TMPFILE"
  fi

  file=$(find_best_conf "$variant_dir"'insertion_*/*/boltz_results_*/predictions/*/confidence_*.json')
  if [ -n "$file" ] && [ -f "$file" ]; then
    dir=$(dirname "$file")
    model=$(echo "$file" | sed 's/.*\(model_[0-9]*\).*/\1/')
    side=$(echo "$file" | sed 's/.*\(insertion_[a-z]*\).*/\1/')
    pae=$(find "$dir" -name "pae_*${model}.npz" ! -name "._*" | head -1)
    cif=$(find "$dir" -name "*${model}.cif" ! -name "._*" | head -1)
    plddt_f=$(find "$dir" -name "plddt_*${model}.npz" ! -name "._*" | head -1)
    [ -n "$pae" ] && echo "${variant}|${side}|${pae}|${file}|${plddt_f}|${cif}" >> "$TMPFILE"
  fi
done
...
for entry in entries:
    ...
    # Collect stats
    m_run = re.search(r'run_(\d+)', conf_path)
    m_model = re.search(r'model_(\d+)', conf_path)
    run = f"run_{m_run.group(1)}" if m_run else "?"
...
# === Summary markdown ===
md = []
md.append("# Boltz-2 Prediction Results — Best Models Summary")
md.append("")
md.append("Pembrolizumab (5DK3) with linker variants. Best model per variant/strategy from 5×50 diffusion samples.")
md.append("")
md.append("## Overview")
md.append("")
md.append("| Variant | Strategy | Confidence | pTM | ipTM | pLDDT | Source |")
md.append("|---------|----------|-----------|------|------|-------|--------|")
for d in sorted(all_stats, key=lambda x: -x["conf"]):
    md.append(f"| {d['variant']} | {d['strategy']} | **{d['conf']:.4f}** | {d['ptm']:.4f} | {d['iptm']:.4f} | {d['plddt']:.4f} | {d['source']} |")

md.append("")
md.append("## Per-Variant Details")
md.append("")

for d in all_stats:
    md.append(f"### {d['variant']} — {d['strategy']}")
    md.append("")
    ...
    if pair and d["chains"]:
        ...
        md.append("")
    md.append(f"![{d['variant']} {d['strategy']}]({d['img']})")
    md.append("")
    md.append("---")
    md.append("")

with open("best_pae_plots/summary.md", "w") as f:
    f.write("\n".join(md))
print("\n  Saved: best_pae_plots/summary.md")
print("\nDone!")
PYEOF
